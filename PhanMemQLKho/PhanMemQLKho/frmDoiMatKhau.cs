        ...
        private void btnDoiMatKhau_Click(object sender, EventArgs e)
        {
            ...
            if(txtTenTaiKhoan.Text.Length>0 && txtMatKhau.Text.Length>0 &&txtMatKhauMoi.Text.Length>0 &&txtNhapLaiMKMoi.Text.Length>0)
            {
                try
                {
                    ...
                    myConnection.Open();
                    string strCauTruyVan = "select count(*) from [User] where TenDangNhap=@acc and MatKhau =@pass";
                    ...
                    if (x == 1)
                    {
                        if (kq == 0)
                        {
                            string luumk = "update [User] set MatKhau='" + txtMatKhauMoi.Text + "' where TenDangNhap='" + txtTenTaiKhoan.Text + "'";
                            myConnection = new SqlConnection(chuoiKetNoi);
                            myConnection.Open();
